 ...
 */
class RegularEncounterInstMultiplePaymentIntegrationTest extends UtilsQuery {
    ...
    void copyClaimsToConnectors() {
        ...
        def TC1a = new File(testDataPath + "RegularEncounters/837_TC4_InstOutpatient.dat")
        Assert.assertTrue(TC1a.exists(), testDataPath + "RegularEncounters/837_TC4_InstOutpatient.dat doesn't exist!")
        ...
        def TC1b = new File(testDataPath + "/RegularEncounters/837_TC4_InstOutpatient.dat.properties")
        Assert.assertTrue(TC1b.exists(), testDataPath + "/RegularEncounters/837_TC4_InstOutpatient.dat.properties doesn't exist!")
        def text1 = TC1b.getText()
        def template1 = engine.createTemplate(text1).make(binding)

        createNewFile(inboundFileConnector + "\\837_TC4_InstOutpatient.dat.properties", template1)
        createNewFile(inboundFileConnector + "\\837_TC4_InstOutpatient.dat", template)
...
//        initialClaimAttEDI = new StringReader(new String(claimAtt))
    ...
    void copyPayment1ToConnectors() {
        ...
        def TC1a = new File(testDataPath + "RegularEncounters/837_TC4_InstOutpatient_835a.dat")
        Assert.assertTrue(TC1a.exists(), testDataPath + "RegularEncounters/837_TC4_InstOutpatient_835a.dat doesn't exist!")
        def text = TC1a.getText()
        ...
        def TC1b = new File(testDataPath + "/RegularEncounters/837_TC4_InstOutpatient_835a.dat.properties")
        Assert.assertTrue(TC1b.exists(), testDataPath + "/RegularEncounters/837_TC4_InstOutpatient_835a.dat.properties doesn't exist!")
        def text2 = TC1b.getText()

        def template2 = engine.createTemplate(text2).make(binding)
        createNewFile(paymentAsClaimUpdate + "\\837_TC4_InstOutpatient_835a.dat", template)
        createNewFile(paymentAsClaimUpdate + "\\837_TC4_InstOutpatient_835a.dat.properties", template2)
...
//        println currentEncTimeSQLFormat;
    ...
    void copyPayment2ToConnectors() {
        currentEncTimeSQLFormat = createCurrentTimeSQLFormat()
        def TC1a = new File(testDataPath + "RegularEncounters/837_TC4_InstOutpatient_835b.dat")
        Assert.assertTrue(TC1a.exists(), testDataPath + "RegularEncounters/837_TC4_InstOutpatient_835b.dat doesn't exist!")
        def text = TC1a.getText()
        ...
        def TC1b = new File(testDataPath + "/RegularEncounters/837_TC4_InstOutpatient_835b.dat.properties")
        Assert.assertTrue(TC1b.exists(), testDataPath + "/RegularEncounters/837_TC4_InstOutpatient_835b.dat.properties doesn't exist!")
        def text2 = TC1b.getText()

        def template2 = engine.createTemplate(text2).make(binding)

        createNewFile(paymentAsClaimUpdate + "\\837_TC4_InstOutpatient_835b.dat.properties", template2)
        createNewFile(paymentAsClaimUpdate + "\\837_TC4_InstOutpatient_835b.dat", template)
...
//        println currentEncTimeSQLFormat;
    ...
    void checkUpdatedCustomFields() {
        ...
        Assert.assertNotNull(lastDate, "There is no LastInternalActivity Date = " + select1)
    ...
    void checkEncounterCustomFields() {
        ...
        activeSql.eachRow(select1) { lastDate = "$it.FieldValue" }
        println lastDate
        Assert.assertNull(lastDate, "There is no LastInternalActivity Date = " + select1)
    ...
    void copyTA1ToConnectors() {
        currentTimeSQLFormat = createCurrentTimeSQLFormat()
        def TC1a = new File(testDataPath + "RegularEncounters/837_TC1_TA1.dat")
        ...
        def binding = ["UniqueID": interchangeControlGroup]
        def engine = new SimpleTemplateEngine()
        def template = engine.createTemplate(text).make(binding)

        createNewFile(receiveInboundAcks + "\\837_TC1_TA1.dat", template)
//        def today = new Date()
//        currentTimeSQLFormat = String.format('%tY-%<tm-%<td %<tH:%<tM:%<tS.%<tL', today)
//        println currentTimeSQLFormat;
        // sleep(sleepTime)
    ...
    void checkTA1Event() {
        ...
        def event = getEvent1("Negative TA1 Acknowledgement received from partner indicating interchange is rejected for control issue", trID.toString(), "343")
    ...
    void copy999ToConnectors() {
        currentTimeSQLFormat = createCurrentTimeSQLFormat()
        def TC1a = new File(testDataPath + "RegularEncounters/837_TC1_999.dat")
        ...
        def binding = ["UniqueID": gcn]
        def engine = new SimpleTemplateEngine()
        def template = engine.createTemplate(text).make(binding)

        createNewFile(receiveInboundAcks + "\\837_TC1_999.dat", template)
//        def today = new Date()
//        currentTimeSQLFormat = String.format('%tY-%<tm-%<td %<tH:%<tM:%<tS.%<tL', today)
//        println currentTimeSQLFormat;
        // sleep(sleepTime)
    ...
    void check999Event() {
        ...
        def event = getEvent1("Negative%", trID, "397")
        Assert.assertNotNull(event, "Update  Event is missing for TransmissionSID =" + encTransmissionSID)
    }
    ...
    void copy277ToConnectors() {
        currentTimeSQLFormat = createCurrentTimeSQLFormat()
        def TC1a = new File(testDataPath + "RegularEncounters/837_TC1_277CA.dat")
        Assert.assertTrue(TC1a.exists(), testDataPath + "RegularEncounters/837_TC1_277CA.dat doesn't exist!")
        def text = TC1a.getText()

        def interControlGroup = "select OriginatorTranIdentifier from ClaimHeader where TransmissionSID=" + encTransmissionSID
        def tcn; activeSql.eachRow(interControlGroup) { tcn = "$it.OriginatorTranIdentifier" }

        def binding = ["UniqueID": tcn, "TargetTrackingID": trackingIdentifier]
        def engine = new SimpleTemplateEngine()
        def template = engine.createTemplate(text).make(binding)

        createNewFile(receiveInboundAcks + "\\837_TC1_277CA.dat", template)
//        def today = new Date()
//        currentTimeSQLFormat = String.format('%tY-%<tm-%<td %<tH:%<tM:%<tS.%<tL', today)
//        println currentTimeSQLFormat;
        // sleep(sleepTime)
    ...
    @Test(dependsOnMethods = "check277GroupIdentifiers", groups = "NewInst277")
    void check277Identifiers() {
        //def group999EncounterGroup=getCorrelationID("397", "AckReconID", currentTransmissionSID)
        def group999EncounterGroup = getCorrelationIDWaiting("397", "TrackingID", currentTransmissionSID)
    ...
    @Test(dependsOnMethods = "check277Identifiers", groups = "NewInst277")
    ...
    @Test(dependsOnMethods = "check277AckReconIDIdentifier", groups = "NewInst277")
    void check277Event() {
        def tr = "select * from Claim where TransmissionSID=" + encTransmissionSID
        def trID; activeSql.eachRow(tr) { trID = "$it.ClaimSID" }
        def event = getEvent1("Accepted status reported and details", trID, "302")
        Assert.assertNotNull(event, "Update  Event is missing for TransmissionSID =" + encTransmissionSID)
    }
