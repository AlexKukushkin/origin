 ...
 */
class LDNSErrorExceptions extends UtilsQuery {
...
//      assert true : "Da ii adevarat"
        ...
        def TC1a = new File(testDataPath + "EncounterExceptions/ECHCF_NewSubmitted_LDNS.xml")
        Assert.assertTrue(TC1a.exists(), testDataPath + "EncounterExceptions/ECHCF_NewSubmitted_LDNS.xml doesn't exist!")
        def text1 = TC1a.getText()

        def TC1b = new File(testDataPath + "/EncounterExceptions/ECHCF_NewSubmitted_LDNS.xml.properties")
        Assert.assertTrue(TC1b.exists(), testDataPath + "/EncounterExceptions/ECHCF_NewSubmitted_LDNS.xml.properties doesn't exist!")
        ...
        createNewFile(inboundInternalClaimUpdates + "\\ECHCF_NewSubmitted_LDNS.xml.properties", template2)
        createNewFile(inboundInternalClaimUpdates + "\\ECHCF_NewSubmitted_LDNS.xml", template1)
        /*def today = new Date()
...
//        sleep(60000)
    ...
    void copyPaymentToConnectors() {
        currentTimeSQLFormat = createCurrentTimeSQLFormat()
        def TC1a = new File(testDataPath + "EncounterExceptions/ECHCF_EncRep_LDNS.xml")
        Assert.assertTrue(TC1a.exists(), testDataPath + "EncounterExceptions/ECHCF_EncRep_LDNS.xml doesn't exist!")
        ...
        def TC1b = new File(testDataPath + "/EncounterExceptions/ECHCF_EncRep_LDNS.xml.properties")
        Assert.assertTrue(TC1b.exists(), testDataPath + "/EncounterExceptions/ECHCF_EncRep_LDNS.xml.properties doesn't exist!")
        def text2 = TC1b.getText()

        def template2 = engine.createTemplate(text2).make(binding)
        createNewFile(inboundInternalClaimUpdates + "\\ECHCF_EncRep_LDNS.xml.properties", template2)
        createNewFile(inboundInternalClaimUpdates + "\\ECHCF_EncRep_LDNS.xml", template)
...
//        sleep(60000)
        List<GroovyRowResult> listClaimSID = getClaimID(currentTransmissionSID)
        Assert.assertNotEquals(listClaimSID.size(), 0, "no claims were received for transmission SID = " + currentTransmissionSID)
        listClaimSID.each {
            item ->
...
//        sleep(60000)
        List<GroovyRowResult> listClaimSID = getClaimID(currentTransmissionSID)
        Assert.assertNotEquals(listClaimSID.size(), 0, "no claims were received for transmission SID = " + currentTransmissionSID)
//        Assert.assertNotEquals(listClaimSID.size(), 0, "The exceptions were not created")
        listClaimSID.each {
            item ->
                def event = getEvent1("Compliance rejection during creation of outbound encounter", item.values().toString().replace("[", "").replace("]", ""), "302")
                print "${item.values()}"
                Assert.assertNotNull(event, "Event is missing for TransmissionSID =" + currentTransmissionSID)
        }
    }
...
//        sleep 60000
    ...
    @Test(dependsOnMethods = "copyPaymentToConnectors", groups = "Encounter")
    void checkException() {
//        sleep 30000
