 ...
 */
class Sc2_Prof_ReceiverID extends UtilsQuery {
    ...
    @Test(dependsOnMethods = "checkIfFileConnectorsExist", groups = "ScenarioProf_ReceiverID", alwaysRun = true)
    void copyClaimsToConnectors() {
        currentTimeSQLFormat = createCurrentTimeSQLFormat()
        def TC1a = new File(testDataPath + "ScenarioProf_ReceiverID/837_Prof.dat")
        Assert.assertTrue(TC1a.exists(), testDataPath + "ScenarioProf_ReceiverID/837_Prof.dat doesn't exist!")
...
/*        def TC1b = new File(testDataPath + "ScenarioInst_ServiceID_HC/837_2016.dat.properties")
        ...
        createNewFile(inboundFileConnector + "\\837_Prof.dat", template)
    }

    /* Get TransmissionSID verify TM DB Schema exists*/

    @Test(dependsOnMethods = "copyClaimsToConnectors", groups = "ScenarioProf_ReceiverID", alwaysRun = true)
...
//        sleep(30000)
    ...
    @Test(dependsOnMethods = "getClaimTransmissionSID", groups = "ScenarioProf_ReceiverID", alwaysRun = true)
    void checkClaimID() {
...
//        sleep(30000)
    ...
    void copyPaymentToConnectors() {
        ...
        def TC1a = new File(testDataPath + "ScenarioProf_ReceiverID/835_Prof.dat")
        Assert.assertTrue(TC1a.exists(), testDataPath + "ScenarioProf_ReceiverID/857_1.dat doesn't exist!")
        def text = TC1a.getText()
        ...
        def TC1b = new File(testDataPath + "ScenarioProf_ReceiverID/835_Prof.dat.properties")
        Assert.assertTrue(TC1b.exists(), testDataPath + "ScenarioProf_ReceiverID/835_Prof.dat.properties doesn't exist!")
        def text2 = TC1b.getText()

        def template2 = engine.createTemplate(text2).make(binding)
        createNewFile(paymentAsClaimUpdate + "\\835_Prof.dat.properties", template2)
        createNewFile(paymentAsClaimUpdate + "\\835_Prof.dat", template)
    }

    /* Verify TransmissionSID verify TM DB Schema exists*/

    @Test(dependsOnMethods = "copyPaymentToConnectors", groups = "ScenarioProf_ReceiverID", alwaysRun = true)
    ...
    @Test(dependsOnMethods = "getEncounterClaimTransmissionSID", groups = "ScenarioProf_ReceiverID", alwaysRun = true)
...
//        sleep(sleepTime)
    ...
    @Test(dependsOnMethods = "checkEncounterCustomFields", groups = "ScenarioProf_ReceiverID", alwaysRun = true)
    void validateAttachment() {
        FileInputStream firstTargetFile = new FileInputStream(new File("./src/test/resources/com/edifecs/qa/Validation/Assertion_Prof_ReceiverID.dat"));
