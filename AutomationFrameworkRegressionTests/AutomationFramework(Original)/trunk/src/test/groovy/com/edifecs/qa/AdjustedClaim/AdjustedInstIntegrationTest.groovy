...
class AdjustedInstIntegrationTest extends UtilsQuery {
...
//    def currentEncTransmissionSID
    ...
    void copyClaimsToConnectors() {
        ...
        def TC1a = new File(testDataPath + "AdjustedCorrection/Ajusted_Inst/837_I_TC1.dat")
        Assert.assertTrue(TC1a.exists(), testDataPath + "AdjustedCorrection/Ajusted_Inst/837_I_TC1.dat doesn't exist!")
        ...
        createNewFile(inboundFileConnector + "\\837_I_TC1.dat", template)
...
//        println currentTimeSQLFormat;
    ...
    void checkCustomFields() {
        ...
        Assert.assertNull(cf1, "There is no Custom field CurrentState=In Progress for GroupIdentifier = " + cf1)
...
//        initialClaimAttEDI = new StringReader(new String(claimAtt))
    ...
    void copyPaymentToConnectors() {
        ...
        def TC1a = new File(testDataPath + "AdjustedCorrection/Ajusted_Inst/835_TC1.dat")
        Assert.assertTrue(TC1a.exists(), testDataPath + "AdjustedCorrection/Ajusted_Inst/835_TC1.dat doesn't exist!")
        def text = TC1a.getText()
        ...
        def TC1b = new File(testDataPath + "/AdjustedCorrection/Ajusted_Inst/835_TC1.dat.properties")
        Assert.assertTrue(TC1b.exists(), testDataPath + "/AdjustedCorrection/Ajusted_Inst/835_TC1.dat.properties doesn't exist!")
...
//        Assert.assertNotEquals(updatedClaimAttEDI, initialClaimAttEDI, "OPS Claim attachment should not be equals")
    ...
    void copy277ToConnectors() {
        currentTimeSQLFormat = createCurrentTimeSQLFormat()
        def TC1a = new File(testDataPath + "AdjustedCorrection/Ajusted_Inst/277.dat")
        Assert.assertTrue(TC1a.exists(), testDataPath + "AdjustedCorrection/Ajusted_Inst/277.dat doesn't exist!")
        def text = TC1a.getText()
...
//        println currentTimeSQLFormat;
    ...
    @Test(dependsOnMethods = "check277Event", groups = "Adjusted")
    void checkUpdatedCustomFields2() {
        //sleep(60000)

        def cf2 = getCustomField("Source", originalTransmissionSID, "302")
        Assert.assertNull(cf2, "There is no Custom field CurrentState=In Progress for GroupIdentifier = " + cf2)

        def cf3 = getCustomFieldWaiting("Disposition", originalTransmissionSID, "302")
        Assert.assertEquals(cf3, "In Progress", "There is no Custom field CurrentState=In Progress for GroupIdentifier = " + cf3)

        def cf4 = getCustomFieldWaiting("ClaimID", originalTransmissionSID, "302")
        Assert.assertNotNull(cf4, "There should not be Custom field for ClaimID with GroupIdentifier = " + cf4)

        def cf5 = getCustomField("EncounterID", originalTransmissionSID, "302")
        Assert.assertNull(cf5, "There should not be Custom field for ClaimID with GroupIdentifier =" + cf5)

        def cf6 = getCustomFieldWaiting("CMSICN", originalTransmissionSID, "302")
        Assert.assertEquals(cf6, "123456789", "There should not be Custom field for EncounterID with GroupIdentifier =" + cf6)

        def cf7 = getCustomFieldWaiting("CMSContractID", originalTransmissionSID, "302")
        Assert.assertEquals(cf7, "R5826", "There should not be Custom field for CMSContractID with GroupIdentifier =" + cf7)

        def cf8 = getCustomFieldWaiting("SubmissionType", originalTransmissionSID, "302")
        Assert.assertEquals(cf8, "Initial", "There should not be Custom field for SubmissionType with GroupIdentifier =" + cf8)

        def cf9 = getCustomField("LastInternalActivityDate", originalTransmissionSID, "302")
        Assert.assertNull(cf9, "There should not be Custom field for LastInternalActivityDate with GroupIdentifier =" + cf9)
    }
...
//        def payerClaimControlNumber =
        ...
        def TC1a = new File(testDataPath + "AdjustedCorrection/Ajusted_Inst/MAO002_TC1.xml")
        Assert.assertTrue(TC1a.exists(), testDataPath + "AdjustedCorrection/Ajusted_Inst/MAO002_TC1.xml doesn't exist!")
        def text = TC1a.getText()
...
//        sleep(20000)
    ...
    void copyAdjustedECHCFToConnectors() {
        currentTimeSQLFormat = createCurrentTimeSQLFormat()
        def TC1a = new File(testDataPath + "AdjustedCorrection/Ajusted_Inst/ECHCF_Update.xml")
        Assert.assertTrue(TC1a.exists(), testDataPath + "AdjustedCorrection/Ajusted_Inst/ECHCF_Update.xml doesn't exist!")
        def text = TC1a.getText()
        ...
        def TC1b = new File(testDataPath + "/AdjustedCorrection/Ajusted_Inst/ECHCF_Update.xml.properties")
        Assert.assertTrue(TC1b.exists(), testDataPath + "/AdjustedCorrection/Ajusted_Inst/ECHCF_Update.xml.properties doesn't exist!")
        def text2 = TC1b.getText()

        def template2 = engine.createTemplate(text2).make(binding)
        createNewFile(inboundInternalClaimUpdates + "\\ECHCF_Update.xml.properties", template2)
        createNewFile(inboundInternalClaimUpdates + "\\ECHCF_Update.xml", template)
...
//        println currentEncTimeSQLFormat;
    ...
    void copyAdjPaymentToConnectors() {
        ...
        def TC1a = new File(testDataPath + "AdjustedCorrection/Ajusted_Inst/835_TC2.dat")
        Assert.assertTrue(TC1a.exists(), testDataPath + "AdjustedCorrection/Ajusted_Inst/835_TC2.dat doesn't exist!")
        def text = TC1a.getText()
        ...
        def TC1b = new File(testDataPath + "/AdjustedCorrection/Ajusted_Inst/835_TC2.dat.properties")
        Assert.assertTrue(TC1b.exists(), testDataPath + "/AdjustedCorrection/Ajusted_Inst/835_TC2.dat.properties doesn't exist!")
        def text2 = TC1b.getText()
