 ...
 */
class PartDenied extends UtilsQuery {
    ...
    void copyClaimsToConnectors() {
        ...
        def TC1a = new File(testDataPath + "DeniedEncounters/837_Part.dat")
        Assert.assertTrue(TC1a.exists(), testDataPath + "DeniedEncounters/837_Part.dat doesn't exist!")
        ...
        def TC1b = new File(testDataPath + "DeniedEncounters/837_Part.dat.properties")
        Assert.assertTrue(TC1b.exists(), testDataPath + "DeniedEncounters/837_Part.dat.properties doesn't exist!")
        def text1 = TC1b.getText()
        def template1 = engine.createTemplate(text1).make(binding)

        createNewFile(inboundFileConnector + "\\837_Part.dat.properties", template1)
        createNewFile(inboundFileConnector + "\\837_Part.dat", template)
...
//        initialClaimAttEDI = new StringReader(new String(claimAtt))
    ...
    void copyPaymentToConnectors() {
        ...
        def TC1a = new File(testDataPath + "DeniedEncounters/835_Part.dat")
        Assert.assertTrue(TC1a.exists(), testDataPath + "DeniedEncounters/835_Part.dat doesn't exist!")
        def text = TC1a.getText()
        ...
        def TC1b = new File(testDataPath + "DeniedEncounters/835_Part.dat.properties")
        Assert.assertTrue(TC1b.exists(), testDataPath + "DeniedEncounters/835_Part.dat.properties doesn't exist!")
        def text2 = TC1b.getText()

        def template2 = engine.createTemplate(text2).make(binding)
        createNewFile(paymentAsClaimUpdate + "\\835_Part.dat.properties", template2)
        createNewFile(paymentAsClaimUpdate + "\\835_Part.dat", template)
...
//        sleep(sleepTime)
    ...
    /*EncounterNotReady folder was created because Source and Target are equal,bug 98148*/
...
//        sleep(30000)
        ...
        Assert.assertEquals(cf3, "In Progress", "There is no Custom field CurrentState=In Delivery for GroupIdentifier = " + cf3)
