...
public class RegularEncounterInstEDIIntegrationTest extends UtilsQuery {
...
//        sleep(60000)
        //todo: maybe we don't have to wait for this??
...
//        listClaimSID.each{
//            item -> def event = getEvent1("Claim was%", item.values().toString().replace("[","").replace("]",""), "302")
//            print "${item.values()}"
...
//        Assert.assertEquals(cf3,"In Progress","There is no Custom field CurrentState=In Progress for GroupIdentifier = "+cf3)
        ...
        //The custom field SubmissionType is updated for the encounter transmission. Updated method call to pass encTransmissionSID
...
//
        def cf9 = "select * from dbo.CFStringValue where CustomFieldSID in (select CustomFieldSID from CustomField where Name='LastInternalActivityDate') and TransmissionSID='" + currentTransmissionSID + "' and InstanceTypeID=302"
        def cf99; activeSql.eachRow(cf9) { cf99 = "$it.FieldValue" }
        Assert.assertNull(cf99, "Custom field LastInternalActivityDate should not be null for GroupIdentifier = " + cf9)
//        def cf9 = getCustomField("LastInternalActivityDate", currentTransmissionSID, "302")
...
//        sleep 30000
    ...
    void checkEncounterOpsData() {
        ...
        initialClaimAttEDI = attList.get(0).getData()
        updatedClaimAttEDI = attList.get(1).getData()
...
//        Assert.assertNotNull(cf9,"There should not be Custom field for LastInternalActivityDate with GroupIdentifier ="+cf9)
    ...
    void copyTA1ToConnectors() {
        currentTimeSQLFormat = createCurrentTimeSQLFormat()
        def TC1a = new File(testDataPath + "RegularEncounters/837I_TC1_TA1.dat")
        ...
        createNewFile(receiveInboundAcks + "\\837I_TC1_TA1.dat", template)
        /*def today = new Date()
        currentTimeSQLFormat = String.format('%tY-%<tm-%<td %<tH:%<tM:%<tS.%<tL', today)
        println currentTimeSQLFormat;*/
    }
    ...
    void copy999ToConnectors() {
        currentTimeSQLFormat = createCurrentTimeSQLFormat()
        def TC1a = new File(testDataPath + "RegularEncounters/837I_TC1_999.dat")
        ...
        createNewFile(receiveInboundAcks + "\\837I_TC1_999.dat", template)
        /*def today = new Date()
        currentTimeSQLFormat = String.format('%tY-%<tm-%<td %<tH:%<tM:%<tS.%<tL', today)
        println currentTimeSQLFormat;*/
    }
...
//        sleep(3000)
    ...
    void copy277ToConnectors() {
        currentTimeSQLFormat = createCurrentTimeSQLFormat()
        def TC1a = new File(testDataPath + "RegularEncounters/837I_TC1_277CA.dat")
        ...
        createNewFile(receiveInboundAcks + "\\837I_TC1_277CA.dat", template)
//        def today = new Date()
//        currentTimeSQLFormat = String.format('%tY-%<tm-%<td %<tH:%<tM:%<tS.%<tL', today)
//        println currentTimeSQLFormat;
    }
...
//    @Test (dependsOnMethods = "checkClaimIDEncounter" , groups = "NewInsClaim")
...
//        activeSql.eachRow(SubmittedClaimID) { trackingIdentifier = "$it.ExternalCorrelationIdValue" }
        def cid = getCorrelationIDWaiting("302", "SubmittedClaimID", encTransmissionSID)
