 ...
 */
class Sc2_Inst_ServiceID_NU extends UtilsQuery {
    ...
    @Test(dependsOnMethods = "checkIfFileConnectorsExist", groups = "ScenarioInst_ServiceID_NU", alwaysRun = true)
    void copyClaimsToConnectors() {
        currentTimeSQLFormat = createCurrentTimeSQLFormat()
        def TC1a = new File(testDataPath + "ScenarioInst_ServiceID_NU/837_2016.dat")
        Assert.assertTrue(TC1a.exists(), testDataPath + "ScenarioInst_ServiceID_NU/837_2016.dat doesn't exist!")
        ...
        createNewFile(inboundFileConnector + "\\837_2016.dat", template)
    }

    /* Get TransmissionSID verify TM DB Schema exists*/

    @Test(dependsOnMethods = "copyClaimsToConnectors", groups = "ScenarioInst_ServiceID_NU", alwaysRun = true)
...
//        sleep(30000)
    ...
    @Test(dependsOnMethods = "getClaimTransmissionSID", groups = "ScenarioInst_ServiceID_NU", alwaysRun = true)
    void checkClaimID() {
...
//        sleep(30000)
    ...
    void copyPaymentToConnectors() {
        ...
        def TC1a = new File(testDataPath + "ScenarioInst_ServiceID_NU/835_2016.dat")
        Assert.assertTrue(TC1a.exists(), testDataPath + "ScenarioInst_ServiceID_NU/857_2016.dat doesn't exist!")
        def text = TC1a.getText()
        ...
        def TC1b = new File(testDataPath + "ScenarioInst_ServiceID_NU/835_2016.dat.properties")
        Assert.assertTrue(TC1b.exists(), testDataPath + "ScenarioInst_ServiceID_NU/835_2016.dat.properties doesn't exist!")
        def text2 = TC1b.getText()

        def template2 = engine.createTemplate(text2).make(binding)
        createNewFile(paymentAsClaimUpdate + "\\835_2016.dat.properties", template2)
        createNewFile(paymentAsClaimUpdate + "\\835_2016.dat", template)
    }

    /* Verify TransmissionSID verify TM DB Schema exists*/

    @Test(dependsOnMethods = "copyPaymentToConnectors", groups = "ScenarioInst_ServiceID_NU", alwaysRun = true)
    ...
    @Test(dependsOnMethods = "getEncounterClaimTransmissionSID", groups = "ScenarioInst_ServiceID_NU", alwaysRun = true)
...
//        sleep(sleepTime)
    ...
    @Test(dependsOnMethods = "checkEncounterCustomFields", groups = "ScenarioInst_ServiceID_NU", alwaysRun = true)
    void validateAttachment() {
        FileInputStream firstTargetFile = new FileInputStream(new File("./src/test/resources/com/edifecs/qa/Validation/Assertion_Inst_NU.dat"));
