package com.edifecs.qa.Regression156629

import com.edifecs.bizitem.data.Attachment
import com.edifecs.bizitem.data.BusinessItem
 ...
 */
class Sc3_Prof_ServiceID_HC extends UtilsQuery {
    ...
    @Test(dependsOnMethods = "checkIfFileConnectorsExist", groups = "ScenarioProf_ServiceID_HC", alwaysRun = true)
    void copyClaimsToConnectors() {
        currentTimeSQLFormat = createCurrentTimeSQLFormat()
        def TC1a = new File(testDataPath + "ScenarioProf_ServiceID_HC/837_1.dat")
        Assert.assertTrue(TC1a.exists(), testDataPath + "ScenarioProf_ServiceID_HC/837_1.dat doesn't exist!")
...
/*        def TC1b = new File(testDataPath + "ScenarioInst_ServiceID_HC/837_2016.dat.properties")
        Assert.assertTrue(TC1b.exists(), testDataPath + "ScenarioInst_ServiceID_HC/837_2016.dat.properties doesn't exist!")
        def text1 = TC1b.getText()
        def template1 = engine.createTemplate(text1).make(binding)

        createNewFile(inboundFileConnector + "\\837_2016.dat.properties", template1)*/
        createNewFile(inboundFileConnector + "\\837_1.dat", template)
    }

    /* Get TransmissionSID verify TM DB Schema exists*/

    @Test(dependsOnMethods = "copyClaimsToConnectors", groups = "ScenarioProf_ServiceID_HC", alwaysRun = true)
...
//        sleep(30000)
    ...
    @Test(dependsOnMethods = "getClaimTransmissionSID", groups = "ScenarioProf_ServiceID_HC", alwaysRun = true)
    void checkClaimID() {
    ...
    @Test(dependsOnMethods = "getClaimTransmissionSID", groups = "ScenarioInst_ServiceID_HC", alwaysRun = true)
...
//        sleep(30000)
    ...
    void copyPaymentToConnectors() {
        ...
        def TC1a = new File(testDataPath + "ScenarioProf_ServiceID_HC/835_1.dat")
        Assert.assertTrue(TC1a.exists(), testDataPath + "ScenarioProf_ServiceID_HC/857_1.dat doesn't exist!")
        def text = TC1a.getText()
        ...
        def TC1b = new File(testDataPath + "ScenarioProf_ServiceID_HC/835_1.dat.properties")
        Assert.assertTrue(TC1b.exists(), testDataPath + "ScenarioProf_ServiceID_HC/835_1.dat.properties doesn't exist!")
        def text2 = TC1b.getText()

        def template2 = engine.createTemplate(text2).make(binding)
        createNewFile(paymentAsClaimUpdate + "\\835_1.dat.properties", template2)
        createNewFile(paymentAsClaimUpdate + "\\835_1.dat", template)
    }

    /* Verify TransmissionSID verify TM DB Schema exists*/

    @Test(dependsOnMethods = "copyPaymentToConnectors", groups = "ScenarioProf_ServiceID_HC", alwaysRun = true)
    ...
    @Test(dependsOnMethods = "getEncounterClaimTransmissionSID", groups = "ScenarioProf_ServiceID_HC", alwaysRun = true)
...
//        sleep(30000)
    ...
    @Test(dependsOnMethods = "getEncounterClaimTransmissionSID", groups = "ScenarioInst_ServiceID_HC", alwaysRun = true)
...
//        sleep(sleepTime)
    ...
    @Test(dependsOnMethods = "checkEncounterCustomFields", groups = "ScenarioProf_ServiceID_HC", alwaysRun = true)
    void validateAttachment() {
        FileInputStream firstTargetFile = new FileInputStream(new File("./src/test/resources/com/edifecs/qa/Validation/Assertion_Prof_HC.dat"));
