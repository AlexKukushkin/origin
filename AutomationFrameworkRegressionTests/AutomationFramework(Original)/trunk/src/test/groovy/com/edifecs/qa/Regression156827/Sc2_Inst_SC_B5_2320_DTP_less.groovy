 ...
 */
class Sc2_Inst_SC_B5_2320_DTP_less extends UtilsQuery {
    ...
    @Test(dependsOnMethods = "checkIfFileConnectorsExist", groups = "Sc_Inst_B5_2320_DTP_date_less_2", alwaysRun = true)
    void copyClaimsToConnectors() {
        currentTimeSQLFormat = createCurrentTimeSQLFormat()
        def TC1a = new File(testDataPath + "Sc_Inst_B5_2320_DTP_date_less_2/837_Inst_2320.dat")
        Assert.assertTrue(TC1a.exists(), testDataPath + "Sc_Inst_B5_2320_DTP_date_less_2/837_Inst_2320.dat doesn't exist!")
...
/*        def TC1b = new File(testDataPath + "ScenarioInst_ServiceID_HC/837_2016.dat.properties")
        ...
        createNewFile(inboundFileConnector + "\\837_Inst_2320.dat", template)
    }

    /* Get TransmissionSID verify TM DB Schema exists*/

    @Test(dependsOnMethods = "copyClaimsToConnectors", groups = "Sc_Inst_B5_2320_DTP_date_less_2", alwaysRun = true)
...
//        sleep(30000)
    ...
    @Test(dependsOnMethods = "getClaimTransmissionSID", groups = "Sc_Inst_B5_2320_DTP_date_less_2", alwaysRun = true)
    void checkClaimID() {
...
//        sleep(30000)
    ...
    void copyPaymentToConnectors() {
        ...
        def TC1a = new File(testDataPath + "Sc_Inst_B5_2320_DTP_date_less_2/835_Inst_2320.dat")
        Assert.assertTrue(TC1a.exists(), testDataPath + "Sc_Inst_B5_2320_DTP_date_less_2/835_Inst_2320.dat doesn't exist!")
        def text = TC1a.getText()
        ...
        def TC1b = new File(testDataPath + "Sc_Inst_B5_2320_DTP_date_less_2/835_Inst_2320.dat.properties")
        Assert.assertTrue(TC1b.exists(), testDataPath + "Sc_Inst_B5_2320_DTP_date_less_2/835_Inst_2320.dat.properties doesn't exist!")
        def text2 = TC1b.getText()

        def template2 = engine.createTemplate(text2).make(binding)
        createNewFile(paymentAsClaimUpdate + "\\835_Inst_2320.dat.properties", template2)
        createNewFile(paymentAsClaimUpdate + "\\835_Inst_2320.dat", template)
    }

    /* Verify TransmissionSID verify TM DB Schema exists*/

    @Test(dependsOnMethods = "copyPaymentToConnectors", groups = "Sc_Inst_B5_2320_DTP_date_less_2", alwaysRun = true)
    ...
    @Test(dependsOnMethods = "getEncounterClaimTransmissionSID", groups = "Sc_Inst_B5_2320_DTP_date_less_2", alwaysRun = true)
...
//        sleep(sleepTime)
    ...
    @Test(dependsOnMethods = "checkEncounterCustomFields", groups = "Sc_Inst_B5_2320_DTP_date_less_2", alwaysRun = true)
    void validateAttachment() {
        FileInputStream firstTargetFile = new FileInputStream(new File("./src/test/resources/com/edifecs/qa/Validation/Assertion_B5_2320_DTP_date_less_2.dat"));
