...
class Sc1_CheckClaim extends UtilsQuery {
    ...
    @Test(dependsOnMethods = "checkIfFileConnectorsExist", groups = "CheckOriginalClaim")
    void copyClaimsToConnectors() {
        currentTimeSQLFormat = createCurrentTimeSQLFormat()
        def TC1a = new File(testDataPath + "Sc1(OriginalClaim)/Claim.xml")
        Assert.assertTrue(TC1a.exists(), testDataPath + "Sc1(OriginalClaim)/Claim.xml doesn't exist!")
        ...
        def TC1b = new File(testDataPath + "/Sc1(OriginalClaim)/Claim.xml.properties")
        Assert.assertTrue(TC1b.exists(), testDataPath + "/Sc1(OriginalClaim)/Claim.xml.properties doesn't exist!")
    ...
    @Test(dependsOnMethods = "copyClaimsToConnectors", groups = "CheckOriginalClaim", alwaysRun = true)
...
//        sleep(30000)
    ...
    @Test(dependsOnMethods = "getClaimTransmissionSID", groups = "CheckOriginalClaim", alwaysRun = true)
    void checkClaimID() {
...
//        sleep(30000)
    ...
    void copyPaymentToConnectors() {
        ...
        def TC1a = new File(testDataPath + "Sc1(OriginalClaim)/Payment.xml")
        Assert.assertTrue(TC1a.exists(), testDataPath + "Sc1(OriginalClaim)/Payment.xml doesn't exist!")
        def text = TC1a.getText()
        ...
        def TC1b = new File(testDataPath + "/Sc1(OriginalClaim)/Payment.xml.properties")
        Assert.assertTrue(TC1b.exists(), testDataPath + "/Sc1(OriginalClaim)/Payment.xml.properties doesn't exist!")
        def text2 = TC1b.getText()
        def template2 = engine.createTemplate(text2).make(binding)
    ...
    @Test(dependsOnMethods = "copyPaymentToConnectors", groups = "CheckOriginalClaim", alwaysRun = true)
...
//        sleep(30000)
        ...
        def cf2 = getCustomField("Source", currentTransmissionSID, "302")
        Assert.assertNotNull(cf2, "There is no Custom field CurrentState=In Progress for GroupIdentifier = " + cf2)
...
//        sleep(sleepTime)
        ...
        Assert.assertEquals(cf7, "182", "There should be Custom field for CMSContractID with GroupIdentifier =" + cf7)
    ...
    @Test(dependsOnMethods = "checkEncounterCustomFields", groups = "CheckOriginalClaim", alwaysRun = true)
