 ...
 */
class RegularEncounter4010NoCorresponding835Integration extends UtilsQuery {
...
//        initialClaimAttEDI = new StringReader(new String(claimAtt))
    ...
    void copyPaymentToConnectors() {
        ...
        def TC1a = new File(testDataPath + "4010RegularEncounters/XML-4010/ECHCF_EncRep_BAD.xml")
        Assert.assertTrue(TC1a.exists(), testDataPath + "4010RegularEncounters/XML-4010/ECHCF_EncRep_BAD.xml doesn't exist!")
        def text = TC1a.getText()
        ...
        def TC1b = new File(testDataPath + "/4010RegularEncounters/XML-4010/ECHCF_EncRep_BAD.xml.properties")
        Assert.assertTrue(TC1b.exists(), testDataPath + "/4010RegularEncounters/XML-4010/ECHCF_EncRep_BAD.xml.properties doesn't exist!")
        def text2 = TC1b.getText()

        def template2 = engine.createTemplate(text2).make(binding)

        createNewFile(inboundInternalClaimUpdates + "\\ECHCF_EncRep_BAD.xml.properties", template2)
        createNewFile(inboundInternalClaimUpdates + "\\ECHCF_EncRep_BAD.xml", template)
...
//        def errorMessage = "select * from ErrorMessage where BizErrorMessage='Encounter Trigger Cancelled - Claim unexpectedly contains a Patient that is not the Subscriber' and TargetInstanceType=302 and TransmissionSID="+currentTransmissionSID
//        print errorMessage
...
//    }
    ...
    void checkAttachmentCreation() {
        //todo: implement waiting method for getAttachment
        sleep(60000)
        List<GroovyRowResult> listClaimSID = getClaimID(currentTransmissionSID)
        listClaimSID.each {
            item ->
